...
public:
	...
	paulstretch_dialog() : myData(), myIsModal(false)
	{
		...
		myCallback = [&](paulstretch_preset data) -> void {
			...
			fb2k::inMainThread([=]() {
					if (!paulstretch_preset::replaceData(newPresetData))
	...
	void OnEnabledCheckBoxChanged(UINT, int, CWindow)
	{
		...
		myCallback(paulstretch_preset(myData.stretchAmount(), myData.windowSize(), myData.enabled()));
	}
...
private:
	...
	bool findPaulstretchData()
	{
		...
		dsp_preset_impl out;
		if (!myDSP_manager_ptr->core_query_dsp(paulstretch_preset::getGUID(), out))
			return false;
		paulstretch_preset paulstretchPreset;
	...
	bool isPaulstretchEnabled()
	{
		myDSP_manager_ptr = static_api_ptr_t<dsp_config_manager>();
		dsp_preset_impl out;
		return myDSP_manager_ptr->core_query_dsp(paulstretch_preset::getGUID(), out);
	}
